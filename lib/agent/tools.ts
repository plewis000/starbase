  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        start_date: { type: "string", description: "Start date in YYYY-MM-DD format (for Gantt view)" },
        is_habit: { type: "boolean", description: "Mark as a habit-task. Enables streak tracking and flexible recurrence (next due from completion date instead of schedule)." },
        recurrence_mode: { type: "string", enum: ["fixed", "flexible"], description: "Recurrence mode: 'fixed' (from due date, default) or 'flexible' (from completion). Auto-inferred from is_habit if not set." },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        is_habit: { type: "boolean", description: "Set habit flag" },
        recurrence_mode: { type: "string", enum: ["fixed", "flexible"], description: "Change recurrence mode" },
      },
      required: ["task_id"],
    },
  },
